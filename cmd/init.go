...
`, name, name)
	...
	if err := os.WriteFile(filepath.Join(dir, "SKILL.md"), []byte(skillMD), 0o644); err != nil { //nolint:gosec // G306: scaffold template files are developer-owned, world-readable is appropriate
		return err
	}
...
`, name, name)
	...
	if err := os.WriteFile(filepath.Join(dir, "server.json"), []byte(serverJSON), 0o644); err != nil { //nolint:gosec // G306: scaffold template files are developer-owned, world-readable is appropriate
		return err
	}
...
`, name)
	...
	if err := os.WriteFile(filepath.Join(dir, ".mcp.json"), []byte(mcpJSON), 0o644); err != nil { //nolint:gosec // G306: scaffold template files are developer-owned, world-readable is appropriate
		return err
	}
...
`, name, name)
	...
	if err := os.WriteFile(filepath.Join(dir, "README.md"), []byte(readmeMD), 0o644); err != nil { //nolint:gosec // G306: scaffold template files are developer-owned, world-readable is appropriate
		return err
	}
...
`, name, titleCase(strings.ReplaceAll(name, "-", " ")))
	...
	if err := os.WriteFile(filepath.Join(claudeDir, "plugin.json"), []byte(claudePlugin), 0o644); err != nil { //nolint:gosec // G306: scaffold template files are developer-owned, world-readable is appropriate
		return err
	}
	if err := os.WriteFile(filepath.Join(codexDir, "plugin.json"), []byte(codexPlugin), 0o644); err != nil { //nolint:gosec // G306: scaffold template files are developer-owned, world-readable is appropriate
		return err
	}
